...
export async function executeTool(toolName, toolInput) {
  switch(toolName) {
    ...
    case "entropy_analyze": {
      ...
      const entropy = -Object.values(freq).reduce((acc,f)=>{const p=f/len;return acc+p*Math.log2(p);},0);
      return {entropy: entropy.toFixed(3), length: text.length, unique_chars: Object.keys(freq).length};
    }
    case "flag_detect": {
      const patterns = [
    ...
    case "wordlist_generate": {
      ...
      const words = new Set([base, base.toLowerCase(), base.toUpperCase()]);
...
export async function agentLoop(messages, onToolCall, onToolResult) {
  ...
  while (iterations < MAX_ITER) {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({model: "claude-opus-4-1-20250805", max_tokens: 4096, tools: AGENT_TOOLS, messages: history, system: SYSTEM_PROMPT}),
    });
    if (!response.ok) {
